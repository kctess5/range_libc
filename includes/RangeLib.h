...
// set this to 1 to use pointer array, 0 to use offset array
#define FLAT_POINTER_INDEX 0
...
namespace ranges {
	...
	public:
		...
		virtual float calc_range(float x, float y, float heading) {
			#if USE_CUDA == 1
		...
		virtual void maybe_warn(int num_casts) {
			#if USE_CUDA == 1
		...
		virtual void calc_range_many(float *ins, float *outs, int num_casts) {
			#if USE_CUDA == 1
			...
			for (int i = 0; i < iters; ++i) {
				...
				calc_range_chunk(&ins[i*CHUNK_SIZE*3],&outs[i*CHUNK_SIZE],num_in_chunk);
			}
			#else
			throw std::string("Must compile with -DWITH_CUDA=ON to use this class.");
			#endif
	...
	template <class successor_t = utils::BTreeStructure<256> >
	// template <class successor_t = utils::SortedVector >
	// template <class successor_t = utils::SetStructure >
	class OnlineCDDTCast : public RangeMethod
	{
	public:
		OnlineCDDTCast() : RangeMethod(), theta_discretization(0) {}
		OnlineCDDTCast(OMap m, float mr, unsigned int td) :  RangeMethod(m, mr), theta_discretization(td) { 
			...
			for (a = 0; a < theta_discretization; ++a)
			{
				std::vector<successor_t > projection_lut;
				for (i = 0; i < lut_widths[a]; ++i)
				{
					successor_t column;
					projection_lut.push_back(column);
		...
		float ANIL calc_range(float x, float y, float heading) {
			...
			// the angle is in range pi:2pi, so we must search in the opposite direction
			if (is_flipped) {
				float nearest = compressed_lut[angle_index][lut_index].prev(lut_space_x);
				if (nearest == std::numeric_limits<float>::min()) {
					return -1.0;
				} else {
					return lut_space_x - nearest;
				}
			} else {
				float nearest = compressed_lut[angle_index][lut_index].next(lut_space_x);
				if (nearest == std::numeric_limits<float>::min()) {
					return -1.0;
				} else {
					return nearest - lut_space_x;
				}
			}
			// this should never occur, if it does, there's an error
			assert(0);
		...
		bool add_obstacle(int x, int y) {
			// add to the omap grid
			map.grid[(int)x][(int)y] = 1;

			int upper_bin = 0, lower_bin = 0;
			float half_lut_space_width = 0, lut_space_center_x = 0, lut_space_center_y = 0;
			float cosangle = 0, sinangle = 0, angle = 0;
			int a, i;
			...
			for (a = 0; a < theta_discretization / 2.0; ++a) {
				...
				sinangle = sin_values[a];
				#else
				angle = M_2PI * a / theta_discretization;
				// angle = angles[a];
				cosangle = cosf(angle);
				sinangle = sinf(angle);
				#endif
				...
				for (i = lower_bin; i <= upper_bin; ++i) 
					compressed_lut[a][i].insert(lut_space_center_x);
			}
			return true;
		}

		bool remove_obstacle(int x, int y) {
			// add to the omap grid
			map.grid[(int)x][(int)y] = 0;
			...
			for (a = 0; a < theta_discretization / 2.0; ++a) {
				...
				for (i = lower_bin; i <= upper_bin; ++i) 
					compressed_lut[a][i].remove(lut_space_center_x);
			}
			return true;
		}
	// protected
		...
		// compressed_lut[theta][offset] -> list of obstacle positions
		std::vector<std::vector<successor_t > > compressed_lut;
		// cached list of y translations necessary to project points into lut space
		std::vector<float> lut_translations;
...
namespace benchmark {
	...
	public:
		...
		double random_sample(int num_samples, bool verbose = true) {
